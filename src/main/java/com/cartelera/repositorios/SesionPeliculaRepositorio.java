...
@Query("SELECT sesion.fecha FROM SesionPelicula sesion WHERE sesion.pelicula.id = :idPelicula AND (sesion.fecha > CURRENT_DATE OR (sesion.fecha = CURRENT_DATE AND sesion.horaInicio > CURRENT_TIME))  ORDER BY sesion.fecha")
...
@Query("SELECT sesion FROM SesionPelicula sesion  WHERE sesion.fecha = :fecha  AND sesion.pelicula.id = :idPelicula AND (sesion.fecha > CURRENT_DATE OR (sesion.fecha = CURRENT_DATE AND sesion.horaInicio > CURRENT_TIME)) ORDER BY sesion.horaInicio")
